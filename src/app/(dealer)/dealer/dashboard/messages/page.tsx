...
const MOCK_THREADS = [
  { id: 1, name: "Suresh Mehta", message: "Is the property still available?", time: "2h ago", unread: true },
  { id: 2, name: "Aishwarya Rao", message: "Can we schedule a site visit?", time: "1d ago", unread: false },
  { id: 3, name: "Ramesh Kumar", message: "What is the maintenance charge?", time: "2d ago", unread: false },
];
...
export default function DealerMessagesPage() {
  ...
  const [chats, setChats] = useState<Record<number, { from: "me" | "them"; text: string }[]>>({
    1: [
      { from: "them", text: "Is the property still available?" },
      { from: "me", text: "Yes! Which property are you inquiring about?" },
    ],
  ...
  const handleSend = () => {
    ...
    setChats((prev) => ({
      ...prev,
      [active]: [...(prev[active] || []), { from: "me", text: newMsg }],
    }));
    setNewMsg("");
  };
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-5xl mx-auto text-charcoal h-[calc(100vh-120px)] flex flex-col">
      <DashboardPageHeader
        ...
        description="Direct inquiries and chats from interested buyers"
            ...
            {MOCK_THREADS.map((t) => (
              <button
                ...
                className={`w-full flex items-center gap-3 p-4 text-left border-b border-indigo/5 hover:bg-indigo/5 transition-colors cursor-pointer ${
                  active === t.id ? "bg-indigo/5" : ""
                }`}
              >
                <Avatar name={t.name} size="md" shape="rounded" tone="indigo" />
          ...
          {active ? (
            <>
              <div className="p-4 border-b border-indigo/5 bg-white/60">
                <p className="text-sm font-bold text-charcoal">
                  {MOCK_THREADS.find((t) => t.id === active)?.name}
                </p>
              </div>
              ...
              <div className="p-4 border-t border-indigo/5 bg-white flex gap-3">
                <TextInput
                  ...
                  className="flex-1"
                />
                <Button variant="secondary" size="sm" onClick={handleSend} aria-label="Send">
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <EmptyState
                ...
                description="Select a buyer from the list to view chat logs"
