...
export default function MyVisitsPage() {
  ...
  const handleCancelVisit = (id: string) => {
    if (confirm("Are you sure you want to cancel this scheduled property visit?")) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 relative">
      ...
      {displayVisits.length === 0 ? (
        <div className="bg-white/80 border border-indigo/10 rounded-3xl p-16 text-center shadow-lg">
          <Calendar className="w-12 h-12 text-indigo/25 mx-auto mb-4" />
          <p className="text-charcoal/50 font-bold text-sm">No scheduled visits found here.</p>
          <Link href="/listings" className="inline-block mt-4 px-6 py-2.5 bg-indigo text-white text-xs font-black uppercase tracking-wider rounded-xl hover:bg-indigo-hover transition-colors shadow">
            ...
            <div key={visit.id} className="bg-white border border-indigo/10 rounded-3xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg border ${STATUS_STYLE[visit.status]}`}>
                    {visit.status}
                  </span>
