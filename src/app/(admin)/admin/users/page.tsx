  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h1 className="text-2xl font-serif font-black text-white">User Management</h1>
          <p className="text-white/40 text-sm font-semibold mt-1">{mockUsers.length} registered users</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search users..." className="w-full bg-[#1e2028] border border-white/10 focus:border-terracotta/50 text-white placeholder-white/30 text-xs font-semibold px-4 py-2.5 pl-10 rounded-xl focus:outline-none" />
        </div>
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="bg-[#1e2028] border border-white/10 text-white/70 text-xs font-semibold px-3 py-2.5 rounded-xl focus:outline-none cursor-pointer">
      ...
      <div className="bg-[#1e2028] border border-white/10 rounded-2xl overflow-hidden">
                ...
                <tr key={user.id} className="hover:bg-white/2 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-indigo/20 flex items-center justify-center text-indigo font-black text-xs shrink-0">{user.name.charAt(0)}</div>
                      <div>
                        <p className="text-sm font-bold text-white">{user.name}</p>
                        <p className="text-[10px] text-white/40 font-semibold">{user.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg border ${user.status === "active" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border-rose-500/20"}`}>
                      {user.status}
                    </span>
                  </td>
                  <td className="px-5 py-4"><span className="text-xs text-white/50 font-semibold">{user.joinedDate}</span></td>
                  <td className="px-5 py-4"><span className="text-sm font-bold text-white/60">{user.inquiriesCount}</span></td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => toggleStatus(user.id, user.name, user.status)}
                        className={`p-2 rounded-lg transition-all cursor-pointer ${user.status === "active" ? "bg-white/5 hover:bg-rose-500/20 text-white/50 hover:text-rose-400" : "bg-white/5 hover:bg-emerald-500/20 text-white/50 hover:text-emerald-400"}`}
                        title={user.status === "active" ? "Suspend" : "Activate"}>
                        {user.status === "active" ? <UserX className="w-3.5 h-3.5" /> : <UserCheck className="w-3.5 h-3.5" />}
                      </button>
                      <button onClick={() => deleteUser(user.id, user.name)} className="p-2 bg-white/5 hover:bg-rose-500/20 text-white/50 hover:text-rose-400 rounded-lg transition-all cursor-pointer" title="Delete">
        ...
        {filtered.length === 0 && <div className="p-10 text-center text-white/30 text-sm font-semibold">No users match your filters.</div>}
      </div>
    </div>
  );
}
